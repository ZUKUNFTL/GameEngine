...
![image-20240130234304943](image-20240130234304943.png)
...
由097节已经可以读取鼠标位置的颜色缓冲区的值，但是quad的范围读取的值是一个固定的50数字

- 此节目的


读取quad实体内的像素在第二个缓冲区的值，会返回当前实体的ID

- 如何实现


见095的思路二中讨论的，在顶点缓冲布局中添加实体ID，这样每个顶点都有一个自己的EntityID值，再将这个ID值作为第二个缓冲区像素的颜色值，这样就可以成功在当前实体里的每个像素都有这个ID值
...
- 在[glsl](https://so.csdn.net/so/search?q=glsl&spm=1001.2101.3001.7020)的顶点着色器阶段**获取**对应布局的EntityID，并**输出**到[fragment](https://so.csdn.net/so/search?q=fragment&spm=1001.2101.3001.7020)阶段；
...
![img](2c9ba68fdc74ad8271ce3aa768cbef69.png)
...
使用点击事件(事件系统）设置hierarchy面板哪个实体被选中
...
![请添加图片描述](f94e5356437a4d8aaa55ba0927393017.png)

# 80. Vulkan、SPIR-V和新Shader系统
...
​		[SPIR-V](https://www.khronos.org/spir/)是一个Shader的编译器，它会把Shader编译为intermediate byte code。全称叫Standard Portable Intermediate Representation，是一种跨平台的中间语言。

### SPIR-V的使用流程

实际使用的时候，比如我有一个vulkan用的shader，语法是glsl，那么怎么在OpenGL上用呢，具体步骤如下：

1. 可以通过SPIR-V，把它编译为Binary文件
2. 基于这个文件，通过SPIR-V的跨平台编译器，返回一个string，此时就是对应的OpenGL版本的shader(也是个文本文件)，然后直接通过OpenGL来编译即可。
3. 或者基于这个文件，通过SPIR-V的跨平台编译器，获取OpenGL版本的shader编译得到的binary文件，可以把它存到磁盘上，用的时候通过glGetProgramBinary类似的代码读取Shader。
...
![img](7d8e294c80fcbde25d4dc4589cf16aec.png)

- 环境变量

![请添加图片描述](12f6ee129cc049b8a588b6a76d28370c.png)
...
注意，项目要取消静态链接改为**动态链接**了，可能是vulkan的那几个lib文件是动态的
...
- 用OpenGL的API**编译链接**OpenGL版本的glsl的SPIR-V二进制文件，与原始编译链接OpenGL的GLSL着色器代码不太一样
...
![请添加图片描述](ddb59afde1174e5b9b56d593dffeb56e.png)
...
# 82. 内容面板和ImGui拖放+Icon

- 此节目的


为完成101所说的，拖动材质赋予实体，需先完成拖动这个功能，因为项目已经有打开场景函数，所以此节完成拖动场景是否能打开场景。
...
![请添加图片描述](b5fa2a344749449babe047d743eabee2.png)
...
![img](f7d1e27ccc73110eff9936959a17d006.png)

所以[数据源](https://so.csdn.net/so/search?q=数据源&spm=1001.2101.3001.7020)一方设置大小时要+1

```
ImGui::SetDragDropPayload("CONTENT_BROWSER_ITEM", itemPath, (wcslen(itemPath) + 1) * sizeof(wchar_t));
```

![img](1f52270913d96f56aef677e3e0d3485f.png)
...
path.c_str()得到的是wstring类型，也就是宽字节符

- 介绍wchar_t类型

wchar_t是C/cpp的字符类型，一种扩展的存储方式，主要用在国际化程序的实现中，但它不等同与Unicode编码；Unicode编码的字符一般是以wchar_t字符存储。char字符类型只有8个位，只能包含256个字符，而像很多外文字符远超过256个字符，例如：中文、日文、韩文等，这些字符需要占用两个字节空间，所以cpp提出了双字节字符类型wchar_t(或叫宽字符类型)。
**wchar_t宽字符类型定义：**
...
![请添加图片描述](19d6b47c2e3a4a2c837afd90375edb04.png)

![请添加图片描述](0ab63e8e7c2347958396ab6fffd7f343.png)
...
![请添加图片描述](66bb898151754a2f9c3082fe9869f75b.png)
...
- 在[数据源](https://so.csdn.net/so/search?q=数据源&spm=1001.2101.3001.7020)上设置
...
![请添加图片描述](52717129bfe94abd8fc7ad89f60ce8f9.png)

## 未来要修改[shader](https://so.csdn.net/so/search?q=shader&spm=1001.2101.3001.7020)系统
...
![img](7e5f531a893e149a57f9850c1ad5d123.png)
...
![img](ff8b741b710aed5cb97d1b063975bbe9.png)
...
![img](ab2a7ac912706e282ef937962f53d156.png)

- 外padding为2，内padding为0

![img](aa4a14e7346076cd791ba267916212a3.png)

- 透明背景

![请添加图片描述](bf82650193224074971642b179079dc5.png)
...
![img](bbbc2818936426291e31d996e270b5cf-17069390668431.png)
...
![img](a30158497afd48bbc7c40d5ec9fefba1-17069390799403.png)
...
![请添加图片描述](9ccfa59144e9428aa3e39203da09c63c-17069391818235.gif)
...
![img](27eb5575b596aa91574f2534008179e2.png)
...
![请添加图片描述](b0f7cf85bcd7423c81ddfc4a3b32bce8.png)

解决方法

组件Components应该不知道实体类Entity的存在，而ScriptableEntity类include包含实体，所以组件Components需改为不包含ScriptableEntity

但是组件Components内部会使用ScriptableEntity，但这里正好是指针，可以前向声明ScriptableEntity，等要使用ScriptableEntity指针的时候再实际包含ScriptableEntity。
...
运行场景时，注意设置面板的上下文为新场景，停止后要设回编辑场景（旧场景）。
...
在editorlayer中点击运行，执行Scene的[Copy函数](https://so.csdn.net/so/search?q=Copy函数&spm=1001.2101.3001.7020)复制当前的场景，并设置面板的上下文为新场景
...
![img](673b2b3a21810b561e9b4456ea2ddb60.png)
...
![请添加图片描述](5a3c0310f35342dba9753832ff1af702.png)

## Cherno遇到的bug

- [hierarchy](https://so.csdn.net/so/search?q=hierarchy&spm=1001.2101.3001.7020)面板不显示实体
...
- Entity的GetName是获取TagComponent的字符串

```c++
const std::string& GetName() { return GetComponent<TagComponent>().Tag; }
```

- CreateEntity 、CreateEntityWithUUID的参数string都是引用类型


- 当执行auto& tag = entity.AddComponent();代码时候，添加新的TagComponent组件，entt的库会移动组件再内存的位置，所以会导致字符串引用的地址无效
...
![img](745b8baf3371ddce4426ada8454173c4.png)
...
![img](850af940202c9f1df4f34a1efd4be27f.png)

![请添加图片描述](1b73a799c4d44687a4d2fd0475d974b4.png)

## Bug

- 圆背景黑色

![img](decb484c2173508e657f0939a2a3587b.png)
...
- 线条、Rect如何画
  ...
  - 先说quad怎么画
    ...
    [OpenGL](https://so.csdn.net/so/search?q=OpenGL&spm=1001.2101.3001.7020)的API画Quad需要提供索引缓冲区，会有重复的索引被使用

    ![img](d5924c91160b474350767e8b8da42ec9.png)
  ...
  - 线条
    ...
    ![img](4fe73d8d984225eb5929acb9baf99e35.png)

    ![img](b2c9b840cb1d3013eca03762fe5316f2.png)
...
![img](a88c3e729d6ad87ffbdc0f5a30b7582c-17070469812918.png)

![img](e724c71952c07cf388838007327376d2-170704698590610.png)
...
渲染线条需要**融入**当前的[批处理](https://so.csdn.net/so/search?q=批处理&spm=1001.2101.3001.7020)中。
...
![img](b8b54e50c38b92f1abc25cf5a915181c.png)

设置宽度的效果

![img](d189e7085a15741ff9193e0d2027b958.png)

将Quad渲染成Rect的效果

![请添加图片描述](f09e4ca25d094dcdba513e9040890bcb.png)
...
![1.效果3png](47aef66a13cfd71aed58ebf5c8ba1e30-17071129555112.png)
...
在EditorLayer的[Update函数](https://so.csdn.net/so/search?q=Update函数&spm=1001.2101.3001.7020)中，并在渲染完场景的物体后，再调用OnOverlayRender函数

## 关于摄像机移到物体后面效果

![img](2dae46267c755a4a2f67286923f99981.png)
...
![img](e1093f9729c42188db6848a25605b3f5.png)

可见正对物体时，包围盒在物体的**前面**。

- 当摄像机移到物体后面时

![img](197a11afda45d5d7ea28a68326956a56.png)
...
![img](4f841290d962bff83dda0a137e35aeba.png)

![img](af83edaad4b50104777d6bc1d5c48be3.png)

即使圆物体**本身边缘**的**alpha为0**，圆包围盒依旧看不清，应该是[glsl](https://so.csdn.net/so/search?q=glsl&spm=1001.2101.3001.7020)的问题吧？
...
![image-20230809232844483](ffdfc156411434f1c1aa5fd8f47d7bbb.png)
...
![img](27ad401e9b5102609b3162ed7a0dac20.png)
...
![请添加图片描述](aa4fdbfe953c4a28b50b289b8cf3cf99.gif)

![img](ff649f8f522ea1224c4f9e565f74cfdb.png)
...
![image-20230809221952678](7d275500b62e91b4c30b9675da299c98.png)
...
![请添加图片描述](c45a3d8e29d345ef8102e8988dc9209f.png)
...
![image-20230809222635532](1f169ae28c93c69d045d95ba6e75eed2.png)
...
![img](8e80de2ea0574e25a2585c4e07b5baa5.png)
...
![2.1偏移位置先相加再偏移](a1d17b7f858e4e33c92728d8854a31dd.gif)
...
![2.2后偏移](4f6a6bf2b243f0bb15fc78767371d22e.gif)

两个bug同样出现在circle上

# 93. 物理模拟模式

113这节没有新功能，只是说了一些教学视频的改变和引擎的规划
...
![2.图标](97e4a26e5f97513d0d294d33485141c9.png)
...
![img](b2fdb12fd7e89f4d778d02dde609b41b.png)

![img](9edcd712fd288320ada74e25882d5299.png)

![请添加图片描述](6c2a8b4eba8f488eada892ff035d15a6.gif)

# 94. 维护项目和合并，模板、模板参数包、函数参数包、参数包转发、扩展参数包、折叠表达式、lambda函数
...
![请添加图片描述](ae4184e6f4e246ec829547c7d102c742.png)
...
![请添加图片描述](3b037f203a9a43598691fa544d42f9e1.png)
...
![请添加图片描述](3a32dd25d6a442a8a533dfc354cab7bc.png)
...
![img](95e3c3bba8455c8cf273ad2fdfe8118e.png)
...
![请添加图片描述](1d97d3752648459f9f0f532074c23246.png)

# 95. 处理GitHub上的ISSUE

- 此节目的

  如标题，处理[GitHub](https://so.csdn.net/so/search?q=GitHub&spm=1001.2101.3001.7020)上的**pull**请求，以防项目有什么问题

## 关于优化

Cherno说，太注重[缓存优化](https://so.csdn.net/so/search?q=缓存优化&spm=1001.2101.3001.7020)可能会降低性能，因为缓存优化需要在CPU上开辟额外空间，这些空间的消耗可能比，不缓存直接运行代码的±运算的消耗还大。
...
具体解释，参考第[112节](https://blog.csdn.net/qq_34060370/article/details/132266359)
...
- 设置[结构体](https://so.csdn.net/so/search?q=结构体&spm=1001.2101.3001.7020)来**初始化项目**
  - 缘由


启动旧的Sandbox项目运行会报错，因为imgui设置了字体，但是sandbox当前目录下没有字体文件，字体文件在hazelnut（可视化编辑器项目）里。

- 解决思路


不想字体文件两个项目路径存在，所以需要设置Sandbox当前目录为hazelnut的工作目录，这样就可以读取在hazelnut里的字体文件。

这个设置需要根据sandbox和hazelnut两个不同项目来分别设置，并且还有运行的窗口名称，所以融合在一起成为结构体来初始化项目运行程序。
...
用visual studio构建mono项目导出mono静态库，静态链接，而不是像peter一样使用mono安装路径下文件夹下已有的mono库。

（为什么要静态库，因为当前引擎就是用静态链接）

相关链接

Peter的讲解mono（https://github.com/peter1745/peter1745.github.io/blob/main/public/mono-guide/src/SUMMARY.md）

## 必要：安装mono[.Net](https://so.csdn.net/so/search?q=.Net&spm=1001.2101.3001.7020)库
...
![image-20230812181244618](df4e4d7a36a6ac03bf8aa044f5db9b3c.png)

- 这个安装目录

  - 有在下面 可选：生成静态库步骤中对应生成的两个东西

    libmono-static-sgen.lib文件
    libmono-static-sgen.lib文件（函数定义）对应的头文件（函数声明）（在include文件内）

  - 有能够实现cpp脚本调用C#脚本的 .Net4.5版本的程序集和库
...
生成静态库：libmono-static-sgen.lib文件
得到libmono-static-sgen.lib库的使用到的头文件类

- 而在Mono的安装目录下包含了这节所需的这两个，所以


这步是可选的，可以直接复制Mono安装目录下的东西
这步哪部分运行失败的情况下，可以直接复制Mono安装目录下的部分
...
![image-20230812210750245](7107fc8dd295edada4d19fb0b8ef3832.png)

### 生成静态库

- 打开mono开源项目下的sln文件

![img](98143f4281a63ec2c034a5f935052e3c.png)
...
![请添加图片描述](97d44babc25747cca4dd9ccc5cb17ded.png)
...
![请添加图片描述](3b86091558ec4a82a8a89e82984e0012.png)
...
![image-20230812213217115](e3a285f536dc448345e00424422c2b73.png)

### 移动生成的库文件与inlcude文件

- 1.将include/mono文件夹放入引擎下的vendor/mono/include下（函数声明）

![img](e5ec9356cd4d90c3c452dad8de7f32e7.png)

- 2.拷贝生成的mono库文件到vender/mono下（函数定义）

源

![img](0c50ff2351d88333b25a655321e50f3b.png)

目标点

![img](a1ce17aa1185c7d08aa80951befd4fd1.png)
...
![img](97713a602e616d13d8a26c93d45283c2.png)

目标

![img](48970f74f42981b34cb2d285479be354.png)
...
![img](b313eb11784701a58de65bcb020729a4.png)
...
![请添加图片描述](dc24f468dc3a4ec9acb5ce7e6e79b4b0.png)
...
![img](5ee94298661e8808ac461be489e27572.png)
...
![img](8efb529b2592e81e121fb71ba1241b3a.png)
...
![image-20230812215440262](6053799b3bd70eec4facbc80977681c3.png)
...
![image-20230812215726598](902bb4ce93ce86afd68e2068c2e9cf09.png)

10MB

2. mono-2.0-sgen.lib 小

![image-20230812215754375](87598e16005cd23bf6e711a6b63aeb5a.png)
...
![image-20230812220259429](8fd7904a871f62b8c47cf9cb91dd7dab.png)

这个项目只会生成一个lib文件

![image-20230812220341625](2c2e9573bb2318a1482c01971676db6a.png)
...
![img](67b02a45b39355ca664c8b6dc2c3d320.png)
...
- 此节目的
  ...
  - 封装类
    ...
    1. 封装加载构建[mono](https://so.csdn.net/so/search?q=mono&spm=1001.2101.3001.7020)环境、Mono类
    ...
    2. ScriptGlue类，封装，C#调用cpp[内部函数](https://so.csdn.net/so/search?q=内部函数&spm=1001.2101.3001.7020)，需向C#提供的接口，成为类

- 如何实现

  Cherno参考Mono官方文档：https://www.mono-project.com/docs/advanced/embedding/（带有例子）

  完整mono文档：http://docs.go-mono.com/?link=xhtml%3adeploy%2fmono-api-assembly.html
...
![请添加图片描述](a7cd6b8ba648449580fc11bf2a312913.png)
...
![img](d3ac0c6bc4014cb905bd4cbef9841963.png)
...
![img](78dddaaf1f6195c6e6b91fc9b3c95af0.png)
...
![请添加图片描述](f44f2c8c6b484f5d9a0ceaac542850dc.png)
...
- ScriptEngine类
  ...
  封装 加载构建[Mono](https://so.csdn.net/so/search?q=Mono&spm=1001.2101.3001.7020)环境类
...
2. 用运行脚本map存储这些封装的mono类对象（用封装的Mono类实例化）
...
​		C#脚本有UUID后，C#的一个脚本才能 与 拥有这个C#脚本的C++实体联系在一起在运行场景的update函数
...
![请添加图片描述](6c954b24611d47829875e34403faa6f8.gif)

![img](09c0f2eb909e505ef13ef83f560a595b.png)
...
![请添加图片描述](a2ddb543447c488c980dd51b776fbfad.png)
...
- 此节目的
  1. 由上节根据UUID获取实体的[translation](https://so.csdn.net/so/search?q=translation&spm=1001.2101.3001.7020)在根据wsad调**整实体的位置**
  2. 此节优化上节所做，实体的translation应属于实体的TransformComponent，所以此节需要在C#**创建组件类**
  3. 用UUID，C#组件类**对应**cpp的组件类。

- 为完成像[Unity](https://so.csdn.net/so/search?q=Unity&spm=1001.2101.3001.7020)的C#脚本API

​	能在C#[获取当前实体](https://so.csdn.net/so/search?q=获取当前实体&spm=1001.2101.3001.7020)在cpp的Rigidbody2DComponent组件
...
![请添加图片描述](b28c449459014b33be5bec931041a10e.gif)

![请添加图片描述](20e4cd4b01684bb798c9bc92dfc46bff.png)
...
![请添加图片描述](f0ed5be890fc44819dbae4fe0c632647.png)
...
![img](2b6035c28fa5cb95badf2501db67e992.png)
...
- 此节目的
  ...
  将[C#游戏](https://so.csdn.net/so/search?q=C%23游戏&spm=1001.2101.3001.7020)代码作为新工作空间的项目，并**引用**C#核心库的项目
  ...
  像[Unity](https://so.csdn.net/so/search?q=Unity&spm=1001.2101.3001.7020)开发时，Unity有一套自己的dll，我们写脚本需要using UnityEngine;，调用Unity库dll才能实现相应效果
...
- 实现细节
  ...
  - 得到C# dll的[Mono](https://so.csdn.net/so/search?q=Mono&spm=1001.2101.3001.7020)类
...
- 新的工作空间文件路径
  ...
  ![请添加图片描述](60bb285f9e7e49129b27f2f940bfb8d2.png)
...
![img](97df7899ad5e4b9b1f4fa4289601901f.png)
...
![请添加图片描述](e0fa2c37168a4efdbb5ac24daa96210c.png)
...
- 原因
  ...
  %{wks.location} = GameEngine

  所以生成目录为:

  GameEngine\Hazelnut\Resources\Scripts

  但是新Sandbox也有自己的新的工作空间，并且也包含了这个GameEngine-ScriptCore项目

  %{wks.location} = GameEngine\Hazelnut\SandboxProject\Assets\Scripts

  所以这生成目录变成为：

  GameEngine\Hazelnut\SandboxProject\Assets\Scripts\GameEngine-Editor\Resources\Scripts

  - 如下生成路径


![请添加图片描述](7d044e551d4c49f69f9888a1df25076f.png)
...
![img](98220cf29e6350571ba5961af567fc35.png)

![请添加图片描述](d228a65932a14c3f8ea275a7327e147f.png)

# 102. C#脚本属性在编辑器面板可见、Mono热更新dll

## 前言

- 此节目的

​	为完成在[编辑器](https://so.csdn.net/so/search?q=编辑器&spm=1001.2101.3001.7020)面板上可以显示C#脚本类的属性，对属性的值修改，C#脚本会**热更新**（mono4.5api支持）

- 如何实现

​	使用[mono](https://so.csdn.net/so/search?q=mono&spm=1001.2101.3001.7020)的api，根据类名获取和设置属性的值

​	参考mono官方api网址：http://docs.go-mono.com/?link=xhtml%3adeploy%2fmono-api-class.html
...
- 实现细节
  ...
  - 考虑设计：
    ...
    视频的方案是设计map<属性name，struct{}>，struct里有属性值等，并将此map放在由加载的C#类抽象的ScriptClass类中，这样并不用考虑序列化数据存储问题，但是每次程序启动都需要获取C#脚本类的属性，存在map中。
...
- 运行游戏时，因120节的**运行脚本map**存储运行时的ScriptInstance指针，用来执行特定需要在OnUpdate中更新的脚本。
  ...
  - ScriptInstance与ScriptClass类
    ...
    1. ScriptInstance类（实例对象Class cl = new Class()）中有对ScriptClass类（抽象类Class）的引用，它们两关系相当于一对多。
    2. ScriptInstance的一个实例对象必有一个ScriptClass对象
    3. ScriptInstance是运行时创建封装的Mono类对象、ScriptClass是加载检测dll创建封装的Mono类。
...
![请添加图片描述](f8c7a1ea41914ac5943d5bd0d8fc2765.png)

2. 游戏运行时为设置speed属性值

![请添加图片描述](b7f050e455f440d78116cdddbb898819.png)

3. 游戏运行时设置speed属性值

![请添加图片描述](88ebe041d31543b99009e63feedb27d1.png)

4. gift效果

​	当调整为0.1的速度，wasd明显受阻，而调整到0.5，速度明显提升（说明[热更新](https://so.csdn.net/so/search?q=热更新&spm=1001.2101.3001.7020)成功）

![1.xiaoguo1232](726a195f3bc7b7478d44f5332858e216.gif)
...
此节完成在运行时在面板可见C#脚本的属性，并且修改属性的值后，可以立马更新C#脚本属性的值

- 缺点


但是运行前（编辑时）并不显示C#脚本的属性，更不能修改。

- 应该要优化地方


应该实现在编辑时能检测C#脚本的属性，并且可以设置值，在运行时读取编辑时设置的值。
...
![请添加图片描述](81a8e4f0100e421fb7e07493d059d246.png)
...
![img](a600f86c8aed29c390ccb52c3a6b309a.png)

![img](563a1b3bf43d0f22e672804bc59f5a8d.png)

- 解决方法


测试后，需将ScriptClass类、ScriptInstance类、ScriptEngine类的顺序声明才行。
...
![请添加图片描述](d3082a8933fb4401880c79d1c9cacb19.png)
